...
def generate_report(username, purchases, registered_users):
    """
    Generates an expense report for a user based on their purchase history.
    """
    ...
    total_item_cost = 0
    most_expensive = purchases[0]
    least_expensive = purchases[0]
    ...
    print("                 -------------------------")
    print(
        f"name: {username}    password: {'*' * len(registered_users[username])}      Tel: +49***20      Date: {time.strftime('%m/%d/%Y')}"
    )
    print("----------------------------------")
    ...
    print(
        f"cost: {most_expensive['total_cost']} EURO          cost: {least_expensive['total_cost']} EURO\n"
    )
    print(f"AVERAGE COST OF ITEM PER ORDER: {average_cost} EURO")
    print(
        f"PURCHASE DATE RANGE: {min(p['date'] for p in purchases)} to {max(p['date'] for p in purchases)}"
    )
    print("--------")
    ...
    if total_item_cost > spending_limit:
        ...
        print(
            "Note: You have not exceeded the spending limit of", spending_limit, "EURO"
        )
